...
class Site_Logo_Image_Control extends WP_Customize_Control {
	...
	public function __construct( $wp_customize, $control_id, $args = array() ) {
		...
		$this->l10n = array(
			'upload' =>      __( 'Add logo', 'ornea' ),
			'set' =>         __( 'Set as logo', 'ornea' ),
			'choose' =>      __( 'Choose logo', 'ornea' ),
			'change' =>      __( 'Change logo', 'ornea' ),
			'remove' =>      __( 'Remove logo', 'ornea' ),
			'placeholder' => __( 'No logo set', 'ornea' ),
		);
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'site-logo-control', get_template_directory_uri() . '/inc/site-logo/css/site-logo-control.css' );
		wp_enqueue_script( 'site-logo-control', get_template_directory_uri() . '/inc/site-logo/js/site-logo-control.js', array( 'media-views', 'customize-controls', 'underscore' ), '', true );
	}
